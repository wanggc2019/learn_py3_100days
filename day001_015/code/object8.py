#!/usr/bin/env python
# -*- coding:utf-8 -*-

"""
面向对象编程 进阶

综合练习 工资结算系统

某公司有三种类型的员工 分别是部门经理、程序员和销售员
需要设计一个工资结算系统 根据提供的员工信息来计算月薪
部门经理的月薪是每月固定15000元
程序员的月薪按本月工作时间计算 每小时150元
销售员的月薪是1200元的底薪加上销售额5%的提成
"""
from abc import ABCMeta, abstractmethod


# 父类
# 以上3类员工皆是雇员
class Employee(object, metaclass=ABCMeta):
    """
    员工
    """
    def __init__(self, name):
        self._name = name

    @property
    def name(self):
        return self._name

    @abstractmethod
    def get_salary(self):
        pass


class Manager(Employee):
    def get_salary(self):
        return 15000.00


class Programmer(Employee):
    def __init__(self, name, working_hours=0):
        super().__init__(name)
        self._working_hours = working_hours

    @property
    def working_hours(self):
        return self._working_hours

    @working_hours.setter
    def working_hours(self, working_hours):
        self._working_hours = working_hours if working_hours > 0 else 0

    def get_salary(self):
        return 150.00 * self._working_hours


class Salesman(Employee):
    def __init__(self, name, sales=0):
        super().__init__(name)
        self._sales = sales

    @property
    def sales(self):
        return self._sales

    @sales.setter
    def sales(self, sales):
        self._sales = sales if sales > 0 else 0

    def get_salary(self):
        return 1200.00 + self._sales * 0.05


def main():
    employees = [Manager("经理"), Programmer("码农小王"), Salesman("销售小陈")]

    for i in employees:
        if isinstance(i, Programmer):
           i.working_hours = int(input("请输入%s本月工作时间：" % i.name))
        elif isinstance(i, Salesman):
            i.sales = int(input("请输入%s本月的销售额：" % i.name))
        """
        同样是接收get_salary这个消息但是不同的员工表现出了不同的行为(多态)
        """
        print("%s本月工资为：￥%s元" % (i.name, i.get_salary()))


if __name__ == '__main__':
    main()
